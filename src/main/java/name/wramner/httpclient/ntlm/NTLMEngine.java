 ...
 * <p>
 * This class has been copied from the Apache HTTP client with very minor modifications (new package, public
 * constructor, built-in Base64 encoder/decoder). It is included with the same license and full source with credits
 * attributed here and in the project's main documentation.
 */
public class NTLMEngine {
    ...
    static final int FLAG_REQUEST_SIGN = 0x00000010; // Requests all messages have a signature attached, in NEGOTIATE
                                                     // message.
    static final int FLAG_REQUEST_SEAL = 0x00000020; // Request key exchange for message confidentiality in NEGOTIATE
                                                     // message. MUST be used in conjunction with 56BIT.
    static final int FLAG_REQUEST_LAN_MANAGER_KEY = 0x00000080; // Request Lan Manager key instead of user session key
    static final int FLAG_REQUEST_NTLMv1 = 0x00000200; // Request NTLMv1 security. MUST be set in NEGOTIATE and
                                                       // CHALLENGE both
    ...
    static final int MSV_AV_FLAGS_ACCOUNT_AUTH_CONSTAINED = 0x00000001; // Indicates to the client that the account
                                                                        // authentication is constrained.
    static final int MSV_AV_FLAGS_MIC = 0x00000002; // Indicates that the client is providing message integrity in the
                                                    // MIC field in the AUTHENTICATE_MESSAGE.
    static final int MSV_AV_FLAGS_UNTRUSTED_TARGET_SPN = 0x00000004; // Indicates that the client is providing a target
                                                                     // SPN generated from an untrusted source.
    ...
    private static byte[] getNullTerminatedAsciiString(final String source) {
        final byte[] bytesWithoutNull = source.getBytes(Charset.forName("US-ASCII"));
    ...
    /**
     * Creates the first message (type 1 message) in the NTLM authentication sequence. This message includes the user
     * name, domain and host for the authentication session.
     *
    ...
    /**
     * Creates the type 3 message using the given server nonce. The type 3 message includes all the information for
     * authentication, host, domain, username and the result of encrypting the nonce sent by the server using the user's
     * password as the key.
     *
    ...
    protected static class CipherGen {
        ...
        public CipherGen(final Random random, final long currentTime, final String domain, final String user,
                        final String password, final byte[] challenge, final String target,
     ...
     *         response field contains the client challenge, null-padded to 24 bytes.
     */
    ...
    private static byte[] lmHash(final String password) throws NTLMEngineException {
        try {
            ...
            final Key lowKey = createDESKey(keyBytes, 0);
            final Key highKey = createDESKey(keyBytes, 7);
            final byte[] magicConstant = "KGS!@#$%".getBytes(Charset.forName("US-ASCII"));
    ...
    static class NTLMMessage {
         ...
         *
         * @param maxlength the maximum length of the response to prepare, including the type and the signature (which
         *        this method adds).
         */
        ...
        /**
         * Returns the response that has been generated after shrinking the array if required and base64 encodes the
         * response.
         *
         * @return The response as above.
         */
        public String getResponse() {
            return new String(Base64.getEncoder().encode(getBytes()), Charset.forName("US-ASCII"));
        }
    ...
    static class Type3Message extends NTLMMessage {
        ...
        Type3Message(final Random random, final long currentTime, final String domain, final String host,
                        final String user, final String password, final byte[] nonce, final int type2Flags,
        ...
        Type3Message(final Random random, final long currentTime, final String domain, final String host,
                        final String user, final String password, final byte[] nonce, final int type2Flags,
                        final String target, final byte[] targetInformation, final Certificate peerServerCertificate,
        ...
        protected void buildMessage() {
            ...
            addULong(
                             ...
                             * 
                             * // Required flags (type2Flags & FLAG_REQUEST_LAN_MANAGER_KEY) | (type2Flags &
                             * FLAG_REQUEST_NTLMv1) | (type2Flags & FLAG_REQUEST_NTLM2_SESSION) |
                             * 
                             * // Protocol version request FLAG_REQUEST_VERSION |
                             * 
                             * // Recommended privacy settings (type2Flags & FLAG_REQUEST_ALWAYS_SIGN) | (type2Flags &
                             * FLAG_REQUEST_SEAL) | (type2Flags & FLAG_REQUEST_SIGN) |
                             * 
                             * // These must be set according to documentation, based on use of SEAL above (type2Flags &
                             * FLAG_REQUEST_128BIT_KEY_EXCH) | (type2Flags & FLAG_REQUEST_56BIT_ENCRYPTION) |
        ...
        /**
         * Add GSS channel binding hash and MIC flag to the targetInfo. Looks like this is needed if we want to use
         * exported session key for GSS wrapping.
         */
    ...
    /**
     * Cryptography support - MD4. The following class was based loosely on the RFC and on code found at
     * http://www.cs.umd.edu/~harry/jotp/src/md.java. Code correctness was verified by looking at MD4.java from the
     * jcifs library (http://jcifs.samba.org). It was massaged extensively to the final form found here by Karl Wright
